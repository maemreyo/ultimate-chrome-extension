// Enhanced settings with multi-provider support and advanced options
...
const PROVIDER_INFO: Record<AIProviderType, ProviderInfo> = {
  ...
  whisper: {
    ...
    requiresApiKey: true,
    models: [{ value: "whisper-1", label: "Whisper Large" }],
...
export function AISettings() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setTestResult({
        success: true,
        message: `Test successful! Response: ${result}`
      })
    } catch (error) {
      setTestResult({
        success: false,
        ...
        <TabsContent value="api-keys" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Alert>
                <AlertDescription>
                  You can configure multiple API keys to enable automatic
                    ...
                    {Object.keys(PROVIDER_INFO)
                      ...
                      .map((provider) => (
                        <Badge
                          key={provider}
                          variant={
                            formData.fallbackProviders?.includes(
        ...
        <TabsContent value="usage" className="space-y-4">
          {stats && (
            <>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {new Date(stats.lastReset).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </CardContent>
                        ...
                        <div key={provider} className="space-y-2">
                          <div className="flex justify-between">
                            <span className="font-medium">
                              {PROVIDER_INFO[provider as AIProviderType]
                                ?.name || provider}
                            </span>
                            <span className="text-sm text-muted-foreground">
                              {providerStats.requests} requests • $
                              {providerStats.cost.toFixed(4)}
                            </span>
                          </div>
                          <Progress
                            value={
                              (providerStats.tokens / stats.tokensUsed) * 100
                            }
                            className="h-2"
                          />
                        </div>
              ...
              {stats.byCapability &&
                Object.keys(stats.byCapability).length > 0 && (
                  <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 gap-4">
                        {Object.entries(stats.byCapability).map(
                          ([capability, count]) => (
                            <div
                              key={capability}
